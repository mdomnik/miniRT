...
static void fan_triangulation(t_group *group, t_obj_file *obj_file, int *indices, int count)
{
    ...
    while (i < count - 1)
    {
        ...
        v2 = indices[i];
        v3 = indices[i + 1];
        if (v1 > 0 && v1 <= obj_file->vertex_count && v2 > 0 && v2 <= obj_file->vertex_count && v3 > 0 && v3 <= obj_file->vertex_count)
        {
            t_triangle triangle = {.p1 = obj_file->vertices[v1], .p2 = obj_file->vertices[v2], .p3 = obj_file->vertices[v3]};
