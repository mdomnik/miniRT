...
int	create_plane(t_world *world, char **args)
{
	...
	color = ft_split(args[3], ',');
	set_transform(pl, translation(ft_atof(coords[0]), ft_atof(coords[1]),
			ft_atof(coords[2])));
	set_transform(pl, rotation_x(deg_to_rad(ft_atof(normal[0])*180)));
	set_transform(pl, rotation_y(deg_to_rad(ft_atof(normal[1])*180)));
	set_transform(pl, rotation_z(deg_to_rad(ft_atof(normal[2])*180)));
