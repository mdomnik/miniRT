...
/*   Updated: 2025/01/14 01:51:27 by mdomnik          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_uv_val cube_uv_back(t_point3 point) {
    t_uv_val uv;
    uv.u = clamp_uv(-point.x + 1.0f);  // Reverse x-axis
    uv.v = clamp_uv(point.y + 1.0f);
    return uv;
}
...
t_uv_val cube_uv_right(t_point3 point) {
    t_uv_val uv;
    uv.u = clamp_uv(1.0f - point.z);  // Reverse z-axis
    uv.v = clamp_uv(point.y + 1.0f);
    return uv;
}
...
t_uv_val cube_uv_up(t_point3 point) {
    t_uv_val uv;
    uv.u = clamp_uv(point.x + 1.0f);
    uv.v = clamp_uv(-point.z + 1.0f);  // Reverse z-axis
    return uv;
}
